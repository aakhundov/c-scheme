...
class terminal {
   public:
    ...
    terminal(const vector<string>& exit_patterns, string history_path = ".history")
        : _history_file{history_path, ios::app},
          _exit_regexes{_make_exit_regexes(exit_patterns)} {
        _load_history(history_path);
    }

    void add_handler(const vector<string>& patterns, handler_type handler);
    void run();

   private:
    vector<regex> _make_exit_regexes(const vector<string>& patterns);
